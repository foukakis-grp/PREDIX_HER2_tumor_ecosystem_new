...
sigdat <- read.csv("/proj/sens2022005/Xenium/PREDIX_HER2/result/scSubtype/NatGen_Supplementary_table_S4.csv")
...
Mydata <- NormalizeData(Mydata)
Mydata <- ScaleData(Mydata, features=temp_allgenes)
Mydata[["RNA"]]$scale.data <- as(Mydata[["RNA"]]$scale.data, Class = "dgCMatrix")

tocalc <- as.data.frame(GetAssayData(Mydata, layer = "scale.data")) 
...
saveRDS(finalm.sweep.t, '/proj/sens2022005/Xenium/PREDIX_HER2/result/scSubtype/scSubtype_result_sep25.rds')
