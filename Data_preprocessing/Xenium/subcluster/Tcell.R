...
table(Idents(data))
data=subset(data,idents=c("T_cells"))
print("Subcluster Tcell")
gc()

## re-integration ##
DefaultAssay(data) <- 'RNA'
data[["RNA"]] <- split(data[["RNA"]], f = data$sampleID)
data <- SCTransform(data, variable.features.n = 2000)
...
save(merged.object, file = "/proj/sens2022005/Xenium/PREDIX_HER2/result/cell_typing/Tcell_subcluster_var2000.Rdata")  



# Further delete some mixed clusters
load("/proj/sens2022005/Xenium/PREDIX_HER2/result/cell_typing/Tcell_subcluster.Rdata")
Idents(merged.object)=merged.object$harm.graph_res.0.8
merged.object = subset(merged.object, idents = c(4,6,8,9,12,13,14), invert = TRUE)
## re-integration ##
data=merged.object
DefaultAssay(data) <- 'RNA'
data[["RNA"]] <- split(data[["RNA"]], f = data$sampleID)
...
save(merged.object, file = "/proj/sens2022005/Xenium/PREDIX_HER2/result/cell_typing/Tcell_subcluster_final.Rdata")  
